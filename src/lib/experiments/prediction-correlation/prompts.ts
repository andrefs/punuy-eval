...
const prompts: PromptGenerator[] = [
  ...
  generate: (vars: Omit<ExpVars, "prompt">): Prompt => {
    ...
    const req =
      p.language === "pt" && p.type === "similarity"
        ? "Por favor avalie a semelhança semântica do seguinte par de palavras"
        : p.language === "pt" && p.type === "relatedness"
          ? "Por favor avalie o relacionamento semântico do seguinte par de palavras"
          : p.language === "en" && p.type === "similarity"
            ? "Please rate the semantic similarity of the following pair of words"
            : "Please rate the semantic relatedness of the following pair of words";
    ...
    return {
      ...
      pairs,
      intro: p.intro,
      followUps: pairs.map(([term1, term2]) => `${req}: ${term1}, ${term2}.`),
    };
  },
}));
