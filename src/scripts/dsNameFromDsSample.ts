...
const nameFromSample = async (ds: DatasetProfile) => {
  ...
  const gpt35turbo_res = await dsNameFromDsSample.perform(10, ds, gpt35turbo, timestamp);
  const gpt4_res = await dsNameFromDsSample.perform(10, ds, gpt4, timestamp);
  const gpt4turbo_res = await dsNameFromDsSample.perform(10, ds, gpt4turbo, timestamp);
