...
const nameFromSample = async (ds: DatasetProfile) => {
  ...
  const gpt35turbo_res = await dsNameFromDsSample.perform(
    trials,
    ds,
    gpt35turbo,
    timestamp
  );
  const gpt4_res = await dsNameFromDsSample.perform(
    trials,
    ds,
    gpt4,
    timestamp
  );
  const gpt4turbo_res = await dsNameFromDsSample.perform(
    trials,
    ds,
    gpt4turbo,
    timestamp
